(function () {
  ...
  class OrganicSource {
    constructor(trackingParamsInstance) {
      let organicSource = this.findByHost(
        extractHostname(trackingParamsInstance.referrer)
      );
    ...
    findByHost(host) {
      ...
      for (let i = 0; i < sources.length; i++) {
        ...
        for (let j = 0; j < subsources.length; j++) {
          ...
          if (subsource.hosts.indexOf(host) > -1)
            return {
              source: source.name,
  ...
  class CustomSource {
    constructor(trackingParamsInstance) {
      ...
      if (!customSource) {
        console.warn(
          "VF Track: UTM data found but not valid for custom source tracking:",
          UTM
        );
        return;
      }
    ...
    find(utmSource) {
      ...
      for (let i = 0; i < allowedSources.length; i++) {
        ...
        for (let j = 0; j < source.subsources.length; j++) {
          ...
          if (subsource.utmSource == utmSource)
            return {
              source: source.name,
              subsource: subsource.name,
            };
  ...
  const isInternalSource = (trackingParamsInstance) => {
    return hostMatches(
      trackingParamsInstance.hostname,
      trackingParamsInstance.referrer
    );
  };
  ...
  class TrackingParams {
    constructor(hostname, referrer, currentURL) {
      ...
      if (!hostMatches(hostname, currentURL))
        throw (
          "VF Track: Cannot instantiate tracking parameters for URL: " +
          currentURL +
